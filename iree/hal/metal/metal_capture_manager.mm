...
Status MetalCaptureManager::Connect() {
  ...
  @autoreleasepool {
    ...
    if (capture_file_ &&
      ...
      LOG(INFO) << "Connected to shared Metal capture manager; writing capture to "
                << std::string([capture_file_.absoluteString UTF8String]);
    } else {
      LOG(INFO) << "Connected to shared Metal capture manager; capturing to Xcode";
    }
  }
...
void MetalCaptureManager::StartCapture() {
  ...
  CHECK(capture_object_) << "Must set capture object before starting";

  LOG(INFO) << "Starting Metal capture";
  @autoreleasepool {
