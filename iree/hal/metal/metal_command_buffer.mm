...
Status MetalCommandBuffer::Dispatch(Executable* executable, int32_t entry_point,
  ...
  DVLOG(2) << "MetalCommandBuffer::Dispatch";
  ...
  for (const auto& pso_kv : pipeline_state_objects_) {
    ...
    DVLOG(3) << "Current pipeline layout: " << pipeline_layout->DebugString();
    CHECK(pso.push_states.size() <= 1);
    CHECK(pso.bind_states.size() <= 1);
    CHECK(pso.constant_states.empty());

    DVLOG(3) << "Encoding push descriptors..";
    for (const auto& push_kv : pso.push_states) {
      ...
      for (const auto& resource_binding : push_state.resource_bindings) {
        DVLOG(3) << "  Resource @[" << resource_binding.DebugStringShort() << "]";
  ...
  DVLOG(2) << "Dispatch workgroup count: (" << workgroups[0] << ", " << workgroups[1]
  ...
  // TODO(antiagainst): release retained object after command buffer completion
