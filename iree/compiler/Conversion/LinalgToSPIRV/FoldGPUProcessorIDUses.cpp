...
IREE::HAL::ExecutableEntryPointOp getEntryPointOp(Operation *op) {
...
IREE::HAL::ReturnOp getEntryPointReturnOp(Operation *op) {
  IREE::HAL::ExecutableEntryPointOp entryPointOp = getEntryPointOp(op);
...
Optional<int64_t> getProcessorIDUpperBound(
  ...
  auto entryPointOp = getEntryPointOp(blockIDOp);
  if (!entryPointOp) return llvm::None;

  int index = blockIDOp.dimensionAttr().getInt();
  auto count = entryPointOp.num_workgroups();
  if (!count) return llvm::None;

  return count->getValue()[index].cast<IntegerAttr>().getInt();
}
