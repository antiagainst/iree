...
/// Given the computation `shape`, whose first element being the slowest varying
/// and last element being the fastest warying dimension, returns the workload
/// values, whose
/// - fastest varying dimension comes first (i.e., GPU x, y, z order);
/// - workload is padded to three dimensions with ones if needed.
static SmallVector<Value, 4> convertToWorkload(OpBuilder &builder, Location loc,
                                               ArrayRef<Value> shape) {
  const unsigned kMinWorkloadDimCount = 3;
  SmallVector<Value, 4> workload = llvm::to_vector<4>(llvm::reverse(shape));
  if (workload.size() < kMinWorkloadDimCount) {
    Value one = builder.create<ConstantIndexOp>(loc, 1);
    workload.resize(kMinWorkloadDimCount, one);
  }
  return workload;
}
...
struct TileAndDistributeOnTensorsPattern
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    count.resize(getNumOuterParallelLoops(op));
    ...
    // XXX: Hack. Gah. This is ugly, but we need it so to make launch
    // configuration query happy. Figure out a proper way to convey this.
    tiledLinalgOp.op.getOperation()->setAttr(
        "iree.codegen.distribution.original_operand_types",
        rewriter.getTypeArrayAttr(op->getOperandTypes()));
    tiledLinalgOp.op.getOperation()->setAttr(
        "iree.codegen.distribution.original_result_types",
        rewriter.getTypeArrayAttr(op->getResultTypes()));
...
struct MakeDispatchWorkgroupsOp : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // kMaxNumParallelDims.
    const unsigned kMaxNumParallelDims = 3;
    if (count.size() > kMaxNumParallelDims) {
      unsigned numSymbols = 0;
      AffineExpr expr = rewriter.getAffineSymbolExpr(numSymbols++);
      for (int64_t i = 1; i < count.size() - kMaxNumParallelDims + 1; i++) {
        expr = expr * rewriter.getAffineSymbolExpr(numSymbols++);
      }
      count[count.size() - kMaxNumParallelDims] = linalg::applyMapToValues(
          rewriter, loc, AffineMap::get(0, numSymbols, expr),
          ArrayRef<Value>(count).take_front(count.size() - kMaxNumParallelDims +
                                            1))[0];
      count = llvm::to_vector<4>(
          ArrayRef<Value>(count).take_back(kMaxNumParallelDims));
    }
    auto workload = convertToWorkload(rewriter, loc, count);
...
void DispatchLinalgOnTensorsPass::runOnOperation() {
  ...
  auto tileSizeFn = [&](OpBuilder &builder,
    ...
    auto numTiledLoops = getNumOuterParallelLoops(cast<linalg::LinalgOp>(op));
