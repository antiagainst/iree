...
// Vulkan GUI + IREE API Integration.
...
int iree::VulkanGuiMain(int argc, char** argv,
                        const VulkanGuiAppInfo& app_info) {
  if (!app_info) {
    LOG(FATAL) << "Invalid VulkanGuiAppInfo";
    return 1;
  }
  ...
  SDL_Window* window = SDL_CreateWindow(
      app_info.app_title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
      app_info.app_width, app_info.app_height, window_flags);
  ...
  LOG(INFO) << "Loading IREE bytecode module...";
  iree_vm_module_t* bytecode_module = nullptr;
  IREE_CHECK_OK(iree_vm_bytecode_module_create(
      iree_const_byte_span_t{
          reinterpret_cast<const uint8_t*>(app_info.bytecode_module.data()),
          app_info.bytecode_module.size()},
  ...
  IREE_CHECK_OK(bytecode_module->lookup_function(
      ...
      iree_string_view_t{app_info.entry_function.data(),
                         app_info.entry_function.size()},
      &main_function));
  iree_string_view_t main_function_name = iree_vm_function_name(&main_function);
  LOG(INFO) << "Resolved main function named '"
            << std::string(app_info.entry_function.data(),
                           app_info.entry_function.size())
            << "'";
  // --------------------------------------------------------------------------
  ...
  while (!done) {
    ...
    // Custom window.
    auto status =
        app_info.imgui_render(iree_vk_device, iree_context, main_function);
    if (!status.ok()) {
      LOG(FATAL) << status;
      done = true;
      continue;
