...
#define DEBUG_TYPE "iree-spirv-winograd-promote"
...
static LogicalResult copyToWorkgroupMemory(OpBuilder &builder, Value src, Value dst) {
...
struct PromoteConstants
    : OpRewritePattern<bufferization::ToMemrefOp> {
  PromoteConstants(MLIRContext *context, PatternBenefit benefit = 1)
      : OpRewritePattern(context, benefit) {}

  LogicalResult matchAndRewrite(bufferization::ToMemrefOp memrefOp, PatternRewriter &rewriter) const override {
    auto memrefType = memrefOp.getType().cast<MemRefType>();
    auto memorySpace = memrefType.getMemorySpaceAsInt();
    if (memorySpace == gpu::GPUDialect::getWorkgroupAddressSpace())
      return failure();
    auto elementType = memrefType.getElementType();
    auto newMemrefType = MemRefType::get(memrefType.getShape(), elementType, {}, gpu::GPUDialect::getWorkgroupAddressSpace());
    rewriter.replaceOpWithNewOp<bufferization::ToMemrefOp>(memrefOp, newMemrefType, memrefOp.getTensor());
    return success();
  }
};
...
struct SPIRVWinogradPromotePass final
    : public SPIRVWinogradPromoteBase<SPIRVWinogradPromotePass> {
...
void SPIRVWinogradPromotePass::runOnOperation() {
  ...
  {
    // Promote constants to shared memory
    RewritePatternSet promotionPatterns(context);
    promotionPatterns.insert<PromoteConstants>(context);
    if (failed(
            applyPatternsAndFoldGreedily(funcOp, std::move(promotionPatterns)))) {
      funcOp.emitOpError() << "failed promoting constants";
...
std::unique_ptr<OperationPass<func::FuncOp>> createSPIRVWinogradPromotePass() {
  return std::make_unique<SPIRVWinogradPromotePass>();
}
