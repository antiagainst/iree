...
static llvm::cl::opt<bool> clCompileToMetalLibrary(
    "iree-metal-compile-to-metal-library",
    llvm::cl::desc("Whether to compile down to .metallib"),
    llvm::cl::init(true));
...
class MetalSPIRVTargetBackend : public TargetBackend {
 public:
  ...
  LogicalResult serializeExecutable(const SerializationOptions &options,
    ...
    if (clCompileToMetalLibrary) {
      // We need to use offline Metal shader compilers. They are only available
      // on macOS devices.
    ...
    if (metalLibs.empty()) {
      ...
      iree_MetalExecutableDef_shader_libraries_add(builder, libsRef);
    }
